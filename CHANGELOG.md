...
- **README 全面专业化改版**（参考 NapCatQQ 等主流 QQ 机器人项目的仓库呈现）：
  - 居中标题 + 一句话定位 + 徽章栏（Release / OneBot v11 / Python / Platform / License）
  - 新增「功能特性」模块表与可折叠完整命令表
  - 新增「架构」图与协议端选型说明
  - 快速开始改为面向新用户的三步流程（clone → 配置 → 双击脚本），不再夹杂本机运维细节
  - 新增「配置」「端口分配」「文档」「注意事项」「免责声明」「License」各节
  - 移除仅与作者本机相关的表述（老项目对照、本机备份路径等），仓库内容对公众读者自洽
- 新增 **LICENSE**（MIT；`SnowLuma/` 目录为独立发行物、遵循其原始许可，不在此覆盖范围）
- 同步仓库简介与 topics（GitHub 仓库设置）

### 说明

- 本版本为纯文档/许可变更，无任何代码行为变化
...
- **`/查看等级`**（别名 `/查看入群等级`、`/等级设置`）：一条命令看全入群自动审批的设置——
  功能状态、等级门槛、生效范围、查不到等级时的 fallback 策略、拒绝理由、通过通知模板，
  并附修改方法提示（群聊，管理员/群主可用）
- **`/查看设置`**（别名 `/全部设置`、`/设置总览`）：所有设置一屏总览——入群审批、
  广告拦截（规则数与禁言时长）、欢迎/退群提示语（超长自动截断防刷屏）、运行范围，
  并列出各设置的修改入口（群聊，管理员/群主可用）

### 变更

- `help.py` 菜单「设置」分组补入上述两条查看命令
- `docs/功能说明.md` 设置命令表同步
